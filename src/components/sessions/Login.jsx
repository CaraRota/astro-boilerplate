...
import { GoogleAuthProvider, getAuth, inMemoryPersistence, signInWithPopup } from 'firebase/auth'
...
const Login = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      successToast('Te has logueado con exito, seras redirigido inmediatamente.')
      if (res.redirected) {
      ...
      <div className='flex h-[300px] min-w-full flex-col rounded-lg border-2 bg-picton-blue-50 text-picton-blue-950 shadow-xl'>
        {isLoading ? (
          ...
          <>
            ...
            <div className='mb-5 border-b border-t border-blue-200 bg-blue-100 px-4 py-3 text-blue-900' role='alert'>
              <p className='font-bold'>Acceso Restringido</p>
              <p className='text-justify text-sm'>
                Luego de registrarte, un administrador deberá aceptar tu solicitud para asignarte como repartidor.
              </p>
            </div>
          </>
        )}
